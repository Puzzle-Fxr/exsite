  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-20 lg:py-28">
      <div className="flex items-center gap-3">
        <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-100 text-cyan-500"><Shield size={24} /></span>
        <span className="text-xs font-semibold uppercase tracking-widest text-cyan-500">Legal</span>
      </div>
      <h1 className="mt-5 text-4xl font-bold text-slate-900">Privacy Policy</h1>
      <p className="mt-3 text-sm text-slate-600">Last updated: {new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
      ...
      <div className="mt-10 space-y-8">
        {sections.map((s) => (
          <section key={s.h}>
            ...
            <p className="mt-3 leading-relaxed text-slate-700">{s.p}</p>
          </section>
        ))}
      </div>
      ...
      <div className="mt-12 rounded-2xl border border-blue-200 bg-blue-50 p-6">
        ...
        <p className="mt-2 text-sm text-slate-700">Questions about this Privacy Policy? Reach us at:</p>
        <p className="mt-2 text-sm text-slate-700">{COMPANY.legalName}</p>
        <p className="text-sm text-slate-700">{COMPANY.address}</p>
        <p className="text-sm text-slate-700">Email: <a href={`mailto:${COMPANY.email}`} className="text-cyan-500 hover:underline">{COMPANY.email}</a></p>
        <p className="text-sm text-slate-700">Phone: {COMPANY.phone}</p>
      </div>
    </div>
  )
}
